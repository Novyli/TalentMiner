...
& $Python -m PyInstaller --noconfirm --clean GQITalentRadar.spec
...
if ($env:WINDOWS_CERT_PATH) {
    ...
    & $SignTool.Source sign /fd SHA256 /td SHA256 /tr http://timestamp.digicert.com `
        /f $env:WINDOWS_CERT_PATH /p $env:WINDOWS_CERT_PASSWORD dist/GQITalentRadar.exe
}
...
if ($env:WINDOWS_CERT_PATH) {
    & $SignTool.Source sign /fd SHA256 /td SHA256 /tr http://timestamp.digicert.com `
        ...
        release/GQI-Talent-Radar-Windows-x64-Setup.exe
}

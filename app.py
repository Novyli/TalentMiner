...
async def lookup_doi_authors(doi: str) -> list[str]:
    """Return the authoritative author list for a DOI when OpenAlex has it."""
    if not doi:
        return []
    import aiohttp
    ...
    try:
        ...
        print(f"DOI author lookup failed for {doi}: {exc}")
        return []
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        doi_authors = await lookup_doi_authors(metadata["doi"])
...
async def crawl_authors(data: dict):
    ...
    try:
        ...
        try:
            save_project(task_id, "", "", "text", ", ".join(authors[:3]))
...
async def export_csv(data: dict):
    ...
    fieldnames = [
        ...
        "cited_by_count", "works_count", "match_score", "sources"
    ]
...
async def search_doi(data: dict):
    ...
    headers = {"User-Agent": "TalentMiner/1.0"}
    try:
        timeout = aiohttp.ClientTimeout(total=15)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, headers=headers) as resp:
